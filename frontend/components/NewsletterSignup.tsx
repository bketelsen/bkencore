...
const NewsletterSignup: FC = () => {
  ...
  const subscribe = async () => {
    ...
    try {
      await DefaultClient.blog.Subscribe({Email: email})
        ...
        <div className="px-6 py-6 bg-indigo-700 rounded-lg md:py-12 md:px-12 lg:py-16 lg:px-16 xl:flex xl:items-center">
          ...
          <div className={`mt-8 sm:w-full sm:max-w-md xl:mt-0 xl:ml-8 ${result?.result === "success" ? "hidden xl:block xl:invisible" : ""}`}>
            <form className="sm:flex">
              ...
              <button
                type="submit"
                className="mt-3 lg:w-36 flex items-center justify-center px-5 py-3 border border-transparent shadow text-base font-medium rounded-md text-white bg-indigo-500 hover:bg-indigo-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-indigo-700 focus:ring-white sm:mt-0 sm:ml-3 sm:w-auto sm:flex-shrink-0"
